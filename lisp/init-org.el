...
(defun get-monday-of-week ()
  "Get the timestamp for Monday of the current week."
  (let* ((c-time (current-time))
         (current-day (if c-time (nth 6 c-time) nil)) ; Day of the week (0-6), 0 is Sunday
         (days-to-monday (if current-day (- current-day 1) nil)))
    (if (and days-to-monday (< days-to-monday 0))
        (setq days-to-monday (+ days-to-monday 7)))
    (if (and c-time days-to-monday)
        (seconds-to-time
         (- (time-to-seconds c-time)
            (* days-to-monday 24 60 60)))
      nil)))
(defun get2-monday-of-week ()
  "Get the timestamp for Monday of the current week."
  (let* ((c-time (current-time))
         (current-day (if c-time (nth 6 c-time) nil)) ; Day of the week (0-6), 0 is Sunday
         (days-to-monday (if current-day (- current-day 1) nil)))
    (message "%s" days-to-monday)
    ))
